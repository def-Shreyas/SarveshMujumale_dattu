...
const aiQuotes = [
  "AI is analyzing your medical records and health data...",
  "Machine learning helps identify injury patterns and trends...",
  "Every medical case tells a story, AI helps us understand it...",
];
...
const SafeMarkdown: React.FC<SafeMarkdownProps> = ({ content }) => {
  ...
  const formatMarkdown = (text: string): string => {
    ...
    // Function to detect and add risk indicators
    const addRiskIndicators = (text: string): string => {
      // Risk patterns - case insensitive
      const riskPatterns = [
        // High/Critical Risk - Red
        { 
          pattern: /\b(high risk|critical risk|severe risk|extreme risk|urgent|critical|severe|dangerous|hazardous)\b/gi, 
        ...
        // Medium Risk - Orange
        { 
          pattern: /\b(medium risk|moderate risk|moderate|warning|caution|significant)\b/gi, 
        ...
        // Low Risk - Green
        { 
          pattern: /\b(low risk|minimal risk|safe|acceptable|controlled|minor|negligible)\b/gi, 
      ...
      // Apply risk indicators (avoid double-wrapping)
      riskPatterns.forEach(({ pattern, class: className, icon }) => {
        processed = processed.replace(pattern, (match) => {
          // Check if already wrapped in a risk badge
          if (processed.includes(`class="${className}"`)) {
            return match;
          }
    ...
    // Function to highlight important information (numbers, percentages, key metrics)
    const highlightImportant = (text: string): string => {
      ...
      // Highlight numbers that might be metrics (standalone numbers or numbers with units)
      text = text.replace(/\b(\d{2,})\b/g, (match) => {
        // Skip if it's part of a date or already highlighted
        if (match.length === 4 && parseInt(match) >= 1900 && parseInt(match) <= 2100) {
          return match; // Likely a year
        }
      ...
      const importantKeywords = [
        'incident', 'accident', 'injury', 'fatality', 'compliance', 'violation',
        'recommendation', 'action required', 'immediate', 'priority', 'key finding',
        'summary', 'conclusion', 'finding', 'observation'
      ];
    ...
    const processTables = (str: string): string => {
      ...
      return str.replace(tableRegex, (match) => {
        ...
        // Build table HTML with enhanced styling
        ...
        // Table body with better formatting
        ...
        dataLines.forEach((line, idx) => {
          ...
          if (cells.length === headers.length) {
            ...
            cells.forEach((cell, cellIdx) => {
              ...
              cellContent = cellContent.replace(/\*\*(.*?)\*\*/g, '<strong class="font-bold text-gray-900">$1</strong>');
              cellContent = cellContent.replace(/\*(.*?)\*/g, '<em class="italic text-gray-700">$1</em>');
              ...
              if (/high|critical|severe|extreme|urgent|dangerous|hazardous/.test(cellLower)) {
                cellClass += ' bg-red-50 border-l-4 border-l-red-500';
              } else if (/medium|moderate|warning|caution|significant/.test(cellLower)) {
                cellClass += ' bg-orange-50 border-l-4 border-l-orange-500';
              } else if (/low|minimal|safe|acceptable|controlled|minor|negligible/.test(cellLower)) {
...
export const Medical: React.FC = () => {
  ...
  const fetchReportText = async (): Promise<string> => {
    ...
    if (!res.ok) {
      ...
      console.error("Fetching /medical-report failed", res.status, raw);
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      const filename = `Dattu_Medical_Report_${timestamp}.pdf`;
    ...
    return (
      <div className="w-full py-12">
        ...
        <div className="text-center mb-10">
          ...
          >
            Upload your Excel medical log data and let DATTU generate a smart,
            interactive dashboard of injury trends, response times, and AI-powered wellness insights.
          </motion.p>
        </div>
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
          {[
            {
              title: "1. Upload Medical Log",
              icon: Upload,
              desc: "Upload your raw First-Aid and LTI case Excel files.",
            },
            {
              title: "2. AI Analyzes Trends",
              icon: Sparkles,
              desc: "DATTU processes injury types, departments, and body parts affected.",
            },
            {
              title: "3. View Dashboard",
              icon: BarChart2,
              desc: "Get interactive charts on injury rates & wellness recommendations.",
            },
          ].map((step, i) => (
            <motion.div
              key={i}
            ...
            <CardHeader className="text-center pb-6">
              ...
              >
                <div className="p-4 rounded-full bg-gradient-to-br from-[#0B3D91]/15 to-[#00A79D]/15 border border-[#0B3D91]/30">
                  ...
                  >
                    <HeartPulse className="w-12 h-12 text-[#0B3D91]" />
                  </motion.div>
                </div>
              </motion.div>
              ...
              <Card className="shadow-lg">
                <div ref={reportContentRef}>
                  <CardHeader>
                    ...
                    <CardDescription>
                      This is the full report generated by the AI based on your uploaded medical records data.
                    </CardDescription>
                  </CardHeader>
                    ...
                    <SelectContent>
                      {chartList.map((chart) => (
                        <SelectItem key={chart.name} value={chart.name}>
                          {chart.name.replace(".html", "").replace(/_/g, " ").replace(/^\d+\s*/, "").replace(/^medical_/i, "")}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
