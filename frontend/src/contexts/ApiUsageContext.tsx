...
export const ApiUsageProvider = ({ children }: { children: ReactNode }) => {
    ...
    useEffect(() => {
        const fetchInitialUsage = async () => {
            try {
                ...
                if (data) {
                    setApiLimit(data.api_calls_limit || 1000);
                    setApiUsed(data.api_calls_used || 0);
                }
            } catch (error) {
    ...
    useEffect(() => {
        ...
        window.fetch = async (...args) => {
            ...
            if (isBackendCall && !isAuthCall && !isDashboardCall) {
    ...
    return (
        <ApiUsageContext.Provider value={{ apiLimit, apiUsed, remainingApi, moduleUsage, incrementUsage }}>
            {children}
